...
{% if grains['os_family'] == 'RedHat' %}
  {% set epel_release = salt['pillar.get']('pkgrepo:epel:release', false) %}
...
{% elif ( grains['saltversion'] >= '2017.7.0' and grains['osmajorrelease'] == 7 ) or ( grains['saltversion'] < '2017.7.0' and grains['osmajorrelease'][0] == '7' ) %}
  {% set pkg = {
    ...
    'rpm': 'http://download.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-' ~ epel_release|default('7-11', true) ~ '.noarch.rpm',
  } %}
...
install_pubkey_epel:
  file.managed:
    ...
    - source: {{ salt['pillar.get']('pkgrepo:epel:pubkey', pkg.key) }}
    - source_hash:  {{ salt['pillar.get']('pkgrepo:epel:pubkey_hash', pkg.key_hash) }}


epel_release:
  pkg.installed:
    - sources:
      - epel-release: {{ salt['pillar.get']('pkgrepo:epel:rpm', pkg.rpm) }}
    - require:
...
{% if salt['pillar.get']('pkgrepo:epel:disabled', False) %}
disable_epel:
  file.replace:
    - name: /etc/yum.repos.d/epel.repo
    - pattern: '^enabled=[0,1]'
...
{% if salt['pillar.get']('pkgrepo:epel:testing', False) %}
enable_epel_testing:
  file.replace:
